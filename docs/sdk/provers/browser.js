...
export class BrowserProver {
    circuitUrl;
    options;
    constructor(circuitUrl, options = {}) {
    ...
    async prove(params) {
        ...
        const pubkey = await fetchDKIMPublicKey(parsed.dkim.selector, parsed.dkim.domain, this.options);
        const witness = await buildWitness(parsed, pubkey, BigInt(params.newOwner), params.nonce);
