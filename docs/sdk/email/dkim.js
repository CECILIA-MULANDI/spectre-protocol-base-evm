...
export async function fetchDKIMPublicKey(selector, domain, options = {}) {
    ...
    if (!resp.ok)
        throw new Error(`DoH lookup failed for ${dnsName}: ${resp.status}`);
    const json = (await resp.json());
    if (!json.Answer?.length)
        throw new Error(`No DKIM TXT record found for ${dnsName}`);
    ...
    if (!match?.[1])
        throw new Error(`No public key (p=) found in DNS for ${dnsName}`);
    return parseSpkiPublicKey(match[1]);
}
async function parseSpkiPublicKey(spkiBase64) {
    ...
    if (!jwk.n || !jwk.e)
        throw new Error("Exported JWK missing RSA modulus or exponent");
    return {
